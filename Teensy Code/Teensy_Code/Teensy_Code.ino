...
void loop()
{
  ...
  {
    if (on && driveMode != 11 && driveMode != 10) {
      updateTempPixels();
    }
    else if (!on) {
      updateTempPixels();
...
{
  if (enabled) {
    repixels.setPixelColor(0, 0, 0, 255);
  }
  else if (startActive) {
    repixels.setPixelColor(0, 255, 0, 0);
  }
  else {
    repixels.setPixelColor(0, 0, 255, 0);
  }
  repixels.show();
}
...
void updateTempPixels()
{
  ...
  if (driveMode == 3)
  {
    ...
    else if (maxCellTemp <= 55) {
      pixels = 4;
    }
    else if (maxCellTemp <= 56) {
      pixels = 6;
    }
    else if (maxCellTemp <= 57) {
      pixels = 8;
    }
    else if (maxCellTemp <= 58) {
      pixels = 10;
    }
    else if (maxCellTemp <= 60) {
      pixels = 12;
    }
  ...
  else
  {
    ...
    else if (maxCellTemp <= 5) {
      pixels = 2;
    }
    else if (maxCellTemp <= 10) {
      pixels = 3;
    }
    else if (maxCellTemp <= 16) {
      pixels = 4;
    }
    else if (maxCellTemp <= 21) {
      pixels = 5;
    }
    else if (maxCellTemp <= 27) {
      pixels = 6;
    }
    else if (maxCellTemp <= 32) {
      pixels = 7;
    }
    else if (maxCellTemp <= 38) {
      pixels = 8;
    }
    else if (maxCellTemp <= 43) {
      pixels = 9;
    }
    else if (maxCellTemp <= 49) {
      pixels = 10;
    }
    else if (maxCellTemp <= 54) {
      pixels = 11;
    }
    else {
      pixels = 12;
...
void processCARCANFrame()
{
  ...
  if (rxmsg.id == 0x20)
  {
    ...
    if (brakePressureFrontF >= 13) {
      brakePressureBool = true;
    }
    else brakePressureBool = false;
    if (brakePressureFrontF >= brakeMax) {
      brakeMax = brakePressureFrontF;
    }
  ...
  else if (rxmsg.id == 0x41)
  {
    ...
    dataByte >>= 1;
    if ((dataByte & 0x01) == 1) {
      prechargeContactor = true;
    }
    else prechargeContactor = false;
    dataByte >>= 1;
    if ((dataByte & 0x01) == 1) {
      negativeContactor = true;
    }
    else negativeContactor = false;
    dataByte >>= 1;
    if ((dataByte & 0x01) == 1) {
      positiveContactor = true;
    }
